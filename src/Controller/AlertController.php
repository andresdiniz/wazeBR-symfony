...
class AlertController extends AbstractController
{
    ...
    {
        $partner = $this->tenantContext->requirePartner();
        $alerts = $this->alertRepo->findAllFilteredByPartnerForExport($partner, $this->filtersFromRequest($request));
        $output = fopen('php://temp', 'w+');
        fputcsv($output, ['ID', 'Tipo', 'Subtipo', 'Via', 'Cidade', 'Publicado', 'Confiança', 'Curtidas', 'Latitude', 'Longitude'], ';');
        foreach ($alerts as $alert) {
            fputcsv($output, [$alert->getId(), $alert->getType(), $alert->getSubtype(), $alert->getStreet(), $alert->getCity(), $alert->getPubMillis(), $alert->getConfidence(), $alert->getNThumbsUp(), $alert->getLatitude(), $alert->getLongitude()], ';');
        }
        rewind($output);
        $content = stream_get_contents($output);
        fclose($output);
        $response = new Response("\xEF\xBB\xBF" . $content);
    ...
    public function live(Request $request): Response
    {
        ...
        arsort($regions);
        $regionRows = array_map(static fn ($city, $count): array => ['city' => $city, 'count' => $count], array_keys($regions), array_values($regions));
    ...
    public function show(int $id): Response
    {
        ...
        return $this->render('alert/show.html.twig', ['partner' => $partner, 'alert' => $alert, 'typesMap' => $this->alertTypeRepo->getTypesMap('pt'), 'subtypesMap' => $this->alertTypeRepo->getSubtypesMap('pt')]);
    }
}
