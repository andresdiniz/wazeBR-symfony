...
export function initDashboard(root = globalThis.document) {
    if (initialized || !root) {
        return;
    }
...
function initFilters(dashboard) {
    ...
    const apply = () => {
        ...
        if (feedback) {
            feedback.textContent = query || section !== 'all'
    ...
    dashboard.querySelectorAll('[data-action="clear-filters"]').forEach((button) => button.addEventListener('click', () => {
        fields.forEach((field) => {
            field.value = field.tagName === 'SELECT'
                ? field.querySelector('option')?.value || 'all'
                : '';
        });
        apply();
    }));
...
function initPagination(dashboard) {
    dashboard.querySelectorAll('[data-pagination]').forEach((pagination) => {
        ...
        const render = () => {
            ...
            rows.forEach((row) => {
                row.hidden = row.dataset.filtered === 'true' || !pageRows.has(row);
            });

            pagination.innerHTML = pages <= 1
                ? ''
                : Array.from({ length: pages }, (_, index) => `<button type="button" class="${index + 1 === page ? 'is-active' : ''}" data-page="${index + 1}">${index + 1}</button>`).join('');
...
function resetPagination(dashboard) {
    dashboard.querySelectorAll('[data-pagination]').forEach((pagination) => pagination._render?.());
}
