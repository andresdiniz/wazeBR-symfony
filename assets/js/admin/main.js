...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Torna L disponível globalmente ──
    window.L = L;

    // ── Inicialização do mapa (show e edit) ──
    const mapContainer = document.getElementById('map');
    if (mapContainer) {
        const textarea = document.getElementById('bbox');
        const isShowPage = document.getElementById('map-show') !== null;
        
        // Se for a página de show, o textarea é um elemento oculto ou não existe
        const wkt = textarea ? textarea.value : (document.getElementById('bbox-wkt')?.textContent || '');
        
        if (!wkt && !isShowPage) {
            // Se não houver WKT e não for show, apenas inicializa o mapa vazio
            return;
        }
        ...
        const map = L.map('map').setView([-20.65, -43.78], 13); // Centro aproximado de Lafaiete
        ...
        // Se for página de show, apenas carrega o WKT e desabilita interação
        if (isShowPage) {
            loadWkt(wkt);
    ...
    // ── Toggle de visualização do token (show) ──
