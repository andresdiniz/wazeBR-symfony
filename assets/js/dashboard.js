(() => {
    ...
    const Dashboard = {
        ...
        renderCharts() {
            ...
            const palette = ['#3379f3', '#f59e0b', '#10b981', '#ef4444', '#8b5cf6', '#0ea5a5'];
            const levelLabels = ['Livre', 'Lento', 'Moderado', 'Pesado', 'Muito Pesado', 'Parado'];

            const subtypeCanvas = document.getElementById('chartAlertsBySubtype');
            if (subtypeCanvas && Array.isArray(data.alertsBySubtype) && data.alertsBySubtype.length) {
                new Chart(subtypeCanvas, {
                    type: 'bar',
                    data: {
                        labels: data.alertsBySubtype.map((item) => item.label),
                        datasets: [{
                            label: 'Alertas',
                            data: data.alertsBySubtype.map((item) => item.total),
            ...
            const jamsCanvas = document.getElementById('chartJamsByLevel');
            if (jamsCanvas && Array.isArray(data.jamsByLevel) && data.jamsByLevel.length) {
                new Chart(jamsCanvas, {
                    type: 'doughnut',
                    data: {
                        labels: data.jamsByLevel.map((_, level) => levelLabels[level] ?? `Nível ${level}`),
                        datasets: [{
                            data: data.jamsByLevel,
            ...
            const streetsCanvas = document.getElementById('chartTopStreets');
            if (streetsCanvas && Array.isArray(data.topStreets) && data.topStreets.length) {
                const top = [...data.topStreets].slice(0, 10).reverse();
                new Chart(streetsCanvas, {
                    type: 'bar',
                    data: {
                        labels: top.map((street) => street.street ?? 'Sem nome'),
                        datasets: [{
                            label: 'Ocorrências',
                            data: top.map((street) => street.occurrences ?? 0),
