/**
 * app-init.js
 * Ponto de entrada principal — carregado via importmap(['app', 'app-init']).
 * Inicializa todos os layouts e componentes assim que o DOM estiver pronto.
 */
...
// ── Módulos exclusivos do dashboard ──────────────────────────────────────────
import { initDashboardClock }    from '../pages/dashboard.js';
import { initDashboardCounters } from '../pages/dashboard.js';
import { initDashboardRefresh }  from '../pages/dashboard.js';
...
export function init(doc = globalThis.document) {
    ...
    if (initialized || !doc) {
        console.debug('[WazeBR] init() ignorado', { initialized, hasDocument: Boolean(doc) });
        return;
    }
    ...
    // Componentes (ordem importa: filters antes de pagination)
    initAccordions(doc);
    initCharts(doc);
    ...
    // Dashboard (só executa se a página tiver [data-dashboard])
    if (doc.querySelector('[data-dashboard]')) {
        initDashboardClock(doc);
        initDashboardCounters(doc);
        initDashboardRefresh(doc);
    }
...
function start() {
    console.debug('[WazeBR] start() executado', { readyState: globalThis.document?.readyState });
