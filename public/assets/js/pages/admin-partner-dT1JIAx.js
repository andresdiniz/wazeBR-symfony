...
function bindDeleteFlow(doc) {
    ...
    const usersTarget    = modal.querySelector('[data-delete-partner-users]');
    const warnBox        = modal.querySelector('[data-delete-partner-warn]');
    ...
    // Sincroniza checkbox visível → hidden do form
    const syncFlag = () => {
        if (!flagInput) return;
    ...
    doc.addEventListener('click', (event) => {
        ...
        const userCount = Number(btn.dataset.partnerUserCount ?? '0');
        ...
        if (usersTarget) usersTarget.textContent = String(userCount);

        const blocked = userCount > 0;

        if (warnBox) warnBox.hidden = !blocked;

        if (feedLabel) feedLabel.hidden = !feed;
        if (feedCheckbox) {
            feedCheckbox.checked = !blocked && Boolean(feed);
            feedCheckbox.disabled = blocked;
        }
