...
function diffToColor(diff) {
    ...
    const d = Math.max(-0.5, Math.min(0.5, diff)); // clamp ±50%
    if (Math.abs(d) < 0.03) return '#e2e8f0';       // empate visual

    if (d > 0) {
        // A pior → vermelho, satura até 0.5
        const t = d / 0.5;
        ...
        mountChart('rcTimeline', canvas, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { grid: { color: COLORS.grid }, beginAtZero: true, title: { display: true, text: 'segundos de atraso' } },
                },
            },
        });
    });
...
function initMap(root) {
    ...
    // B — laranja, com dash para sobrepor visualmente quando coincidem
    if (bPoly.length > 1) {
        L.polyline(bPoly, {
            color: COLORS.orange,
            weight: 5,
...
function initPicker(root) {
    ...
    // Auto-submit quando ambos têm valor e o usuário troca um select
    const autoSubmit = () => {
        if (selA?.value && selB?.value && selA.value === selB.value) {
            // Evita comparar a mesma rota com ela mesma
            return;
        }
    ...
    selA?.addEventListener('change', autoSubmit);
    selB?.addEventListener('change', autoSubmit);
...
export function initRouteCompare(root = document) {
    ...
    // Só renderiza painéis se o "compare" foi preenchido
    if (page.querySelector('[data-rc-heatmap]')) {
        renderHeatmap(page);
        initCharts(page);
        initMap(page);
    }
