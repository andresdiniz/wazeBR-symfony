...
/**
 * Converte ratio (0..1+) em cor HSL verde→amarelo→vermelho.
 * Retorna null se ratio for null.
 */
function ratioToColor(ratio) {
    if (ratio === null || ratio === undefined) return null;
    const r = Math.max(0, Math.min(ratio, 1));
    const hue   = 140 - r * 140;       // 140 verde → 0 vermelho
    const sat   = 62 + r * 12;         // 62 → 74
    const light = 44 - r * 2;          // 44 → 42
...
function initCharts(root) {
    ...
    root.querySelectorAll('[data-chart="route-timeline"]').forEach((canvas) => {
        ...
        mountChart('routeTimeline', canvas, {
            ...
            options: {
                ...
                plugins: {
                    legend: { display: false }, // legend fora, no header
                    tooltip: {
                        callbacks: {
    ...
    root.querySelectorAll('[data-chart="route-jam-dist"]').forEach((canvas) => {
        ...
        // Filtra níveis com 0 ocorrências
