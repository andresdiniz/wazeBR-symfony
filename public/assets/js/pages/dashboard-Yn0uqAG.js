...
export function initDashboardCharts(root = document) {
    ...
    // 7a) Hydro level (12h) — nível do rio + cotas
    ...
    // 7b) Hydro rain (12h) — chuva por hora
...
export function attachDashboardLiveUpdates(root = document) {
    window.addEventListener('dashboard:update', (e) => {
        ...
        // Hydro combined (12h) — alimenta os dois gráficos
        if (data.hydro && data.hydro.combined) {
            const json = JSON.stringify(data.hydro.combined);
            ['[data-chart="hydro-level-12h"]', '[data-chart="hydro-rain-12h"]'].forEach((sel) => {
                const canvas = root.querySelector(sel);
                if (canvas) canvas.dataset.chartData = json;
            });
        }
