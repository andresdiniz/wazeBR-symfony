...
const POLL_INTERVAL    = 30_000;   // fallback só se SSE morrer
const ERROR_THRESHOLD  = 3;
const SOUND_COOLDOWN   = 120_000;

// SSE
const SSE_MAX_FAILURES = 3;        // após 3 falhas seguidas, cai pra polling
...
// ─────────────────────────────────────────────────────────────────────────
// Canal em tempo real: SSE com fallback pra polling
// ─────────────────────────────────────────────────────────────────────────

/**
 * Aplica um payload vindo do backend (SSE ou fetch). Idempotente.
 */
...
function startRealtime() {
    ...
    try {
        eventSource = new EventSource(state.streamEndpoint, { withCredentials: true });
    } catch (err) {
    ...
    eventSource.addEventListener('open', () => {
        console.debug('[TV] SSE conectado');
        sseFailures = 0;
        usingFallback = false;
        hideOverlay();
        stopPolling();
    ...
    eventSource.addEventListener('data', (evt) => {
        try {
            ...
            console.warn('[TV] SSE payload inválido', err);
        }
    });

    eventSource.addEventListener('error', () => {
        // EventSource reentra sozinho graças ao `retry: 3000`.
        // Só caímos pro polling se acumular falhas.
        sseFailures++;

        if (sseFailures >= SSE_MAX_FAILURES) {
            console.warn('[TV] SSE falhou seguido — usando polling');
            stopRealtime();
            startPolling();
        }
    });
...
export function initTvWallboard(root = document) {
    ...
    // Derivamos o endpoint SSE trocando /api/data → /stream
    ...
    // Primeiro frame rápido (JSON one-shot) e logo em seguida SSE
    fetchData().finally(() => {
        startRealtime();
    });

    // Mantém o "há Xs" atualizado sem depender do canal
    setInterval(() => {
    ...
    document.addEventListener('visibilitychange', () => {
        ...
        // Voltou o foco: se ainda estamos no SSE, ótimo.
        // Se caímos pra polling, dá um fetch imediato.
