...
export function attachDashboardLiveUpdates(root = document) {
    window.addEventListener('dashboard:update', (e) => {
        ...
        if (data.kpis) {
            const map = {
                alerts:   data.kpis.alerts   ?? 0,
                jams:     data.kpis.jams     ?? 0,
                weather:  data.kpis.weather  ?? 0,
                cameras:  data.kpis.cameras  ?? 0,
            };
            root.querySelectorAll('.dashboard-kpi-grid .dashboard-kpi-card').forEach((card, i) => {
                const keys = ['alerts', 'jams', 'weather', 'cameras'];
                const el = card.querySelector('.dashboard-kpi-value');
                if (el) el.textContent = String(map[keys[i]] ?? 0);
            });
        }
