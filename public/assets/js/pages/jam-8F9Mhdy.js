 ...
 * Filtros + mapa Leaflet + tabela + gráficos.
 */
...
function renderMap(mapData) {
    ...
    jams.forEach((j) => {
        ...
        poly.bindTooltip(
            ...
            `Nível ${j.level} · ${Math.round(j.delay / 60)} min · ` +
    ...
    // Ajusta o zoom para caber todas as vias
    if (bounds.length >= 2) {
        try {
...
function renderSummary(summary) {
    ...
    setText('[data-jam-stat="level4plus"]',   fmtNum(level4plus));
    setText('[data-jam-stat="level5"]',       fmtNum(s.byLevel?.[5] ?? 0));
...
function renderTable(rows) {
    ...
    tbody.innerHTML = rows.map((j) => `
        <tr>
            <td>
                <span class="jam-table__level jam-table__level--${j.level}">
                    ${j.level}
                </span>
            </td>
            <td>
            ...
            <td class="is-num">${j.delayMin} min</td>
...
function renderRankCities(rows) {
    ...
    el.innerHTML = rows.map((r, i) => `
        <li>
            ...
            <span class="jam-rank__stats">
                ${r.count}
                <small>média ${r.avgDelay}s</small>
            </span>
        </li>
    `).join('');
}
...
export function initJamPage(root = document) {
    ...
    // O mapa é inicializado em paralelo, pode levar 200ms até o Leaflet
    // estar pronto. O primeiro render usa o resultado do fetch.
    initMap();
    fetchAndRender();
