...
/**
 * Retorna { date, time } já no fuso configurado.
 *   date → "16/09/2026"
 *   time → "18:29"
 */
...
/** Só a hora (HH:mm) — usado onde precisa comprimir. */
...
/** Só data curta (dd/mm) — usado no ticker. */
...
/**
 * Deduplica por chave composta. Protege contra backend retornando
 * a mesma linha duas vezes (JOIN sem DISTINCT, união de fontes, etc).
 */
...
function alertKey(a) {
    return a.uuid
        ?? a.id
        ?? `${a.type}|${a.street ?? ''}|${a.city ?? ''}|${a.when ?? ''}`;
}

function feedKey(e) {
    return e.uuid
        ?? e.id
        ?? `${e.kind}|${e.type ?? ''}|${e.street ?? ''}|${e.city ?? ''}|${e.when ?? ''}`;
}
...
function renderTicker(feed) {
    ...
    // duplicação proposital para o loop do keyframe (-50%)
...
function renderCameras(list) {
    ...
    // Mesma lista → não rebuilda (evita piscar a cada poll de 30s)
...
// ─────────────────────────────────────────────────────────────────────────
// Wake Lock — impede a TV de dormir
// ─────────────────────────────────────────────────────────────────────────
...
async function fetchData() {
    ...
    try {
        ...
        renderCameras(json.data.cameras || []);
    } catch (err) {
        state.failures++;
