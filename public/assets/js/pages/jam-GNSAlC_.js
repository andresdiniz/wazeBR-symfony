/**
 * pages/jam.js — Página /jams (padrão alerts)
 *
 * Regras de negócio:
 *   - Interdição:       level = 5  OU  delay = -1
 *   - Interdição ativa: interdição + lastSeen recente
 *   - Interdição antiga: interdição + lastSeen > 15min atrás
 *
 * Export: link CSV com os filtros atuais.
 */
...
function renderMap(mapData) {
    ...
    jams.forEach((j) => {
        ...
        let flag = '';
        if (j.isBlocked && j.isStale) {
            flag = '<br><span style="color:#fdba74;font-weight:800">⚠ INTERDIÇÃO ANTIGA</span>';
        } else if (j.isBlocked) {
            flag = '<br><span style="color:#fca5a5;font-weight:800">🚫 INTERDIÇÃO TOTAL</span>';
        }

        poly.bindTooltip(
            `<strong>${escapeHtml(j.street || 'Via')}</strong><br>` +
            `${escapeHtml(j.city || '')}<br>` +
            `Nível ${j.level} · ${delayLabel} · ${Math.round(j.length / 100) / 10} km` +
            flag,
    ...
    if (hint) hint.textContent = `${jams.length} vias plotadas`;
...
function levelCell(j) {
    if (j.isBlocked && j.isStale) {
        return `<span class="jam-table__stale-badge">⚠ ANTIGA</span>`;
    }
    if (j.isBlocked) {
        return `<span class="jam-table__blocked-badge">🚫 INTERDIÇÃO</span>`;
    }
...
function renderTableBody(tbody, rows, emptyMsg) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="jam-empty">${escapeHtml(emptyMsg)}</td></tr>`;
        return;
    }
...
function renderAllTables(data) {
    ...
    // Atualiza os contadores dos painéis
...
function rankItem(r, idx, kind) {
    ...
    let marks = '';
    if (isActive) {
        marks += `<span class="mark-blocked">🚫 ${r.blockedActive}</span> · `;
    }
    if (Number(r.blockedStale) > 0) {
        marks += `<span class="mark-stale">⚠ ${r.blockedStale}</span> · `;
    }

    let foot = marks;
    if (kind === 'streets') {
        foot += r.maxDelay > 0 ? `máx ${r.maxDelay}s` : '—';
    } else {
        foot += r.avgDelay > 0 ? `média ${r.avgDelay}s` : '—';
    }
...
async function fetchAndRender() {
    ...
    try {
        ...
        const age = data.summary?.lastSeen ? fmtRelative(data.summary.lastSeen) : '—';
